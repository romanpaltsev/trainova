...
def test_prepare_form_has_no_date_and_duration(client, user, bike):
    """Полей нет в самой форме, а не только скрыты в шаблоне."""
    client.force_login(user)

    form = client.get(reverse("cardio_prepare")).context["form"]

    assert "distance_km" in form.fields
    for absent in ("date", "duration_hours", "duration_minutes", "avg_heart_rate"):
        assert absent not in form.fields


def test_prepare_ignores_posted_duration(client, user, bike):
    """Дослать длительность руками и записать тренировку через план нельзя."""
    client.force_login(user)
