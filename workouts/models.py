...
# повторы (вес 0 — «со своим весом»), у удержаний — время. Правило доменное, а не
# экранное: по нему живой режим решает, засчитывать ли «Подход выполнен», запись
# задним числом — можно ли записать черновик, а импорт — брать ли строку из файла.
REQUIRED_FIELD = {
...
def exercise_order_key(first_done_at, first_set_id):
    ...
    Считается в трёх местах — services.exercise_groups (по загруженным строкам),
    stats.exercise_positions и stats.muscle_groups_by_workout (агрегатами в SQL), —
    поэтому правило и вынесено сюда: разъехавшись, они дали бы разные номера одному
