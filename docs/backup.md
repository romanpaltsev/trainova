...
Если же старый сервер жив и переезд плановый — идите по [migration.md](migration.md):
там дамп снимается со старого сервера (а не берётся вчерашний из облака), переносятся
`.env.prod` и конфиг rclone, сертификат выпускается на новом сервере заново, и есть порядок
переключения DNS с откатом. Здесь описан аварийный случай, когда выбора нет.
