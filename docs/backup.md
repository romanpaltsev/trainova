...
там дамп снимается со старого сервера (а не берётся вчерашний из облака), переносится
`.env.prod`, копируется сертификат, и есть порядок переключения DNS с откатом. Здесь
описан аварийный случай, когда выбора нет.
