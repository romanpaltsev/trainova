...
Кратность 16 секундам — характерный шаг ретрансмиссии TCP. Когда очередь accept
переполнена, ядро отбрасывает финальный ACK: у клиента `connect()` возвращается
мгновенно, а первого байта он ждёт до ретрансмита. Отсюда «то быстро, то полминуты».

Команды на сервере, по одной на гипотезу:

```bash
# 1. Очередь accept переполняется? Счётчики ненулевые и растут — это она.
nstat -az | grep -iE 'ListenOverflows|ListenDrops'
ss -lnt | head                       # Recv-Q у :80 и :443

# 2. Таблица conntrack (docker публикует порты через NAT) переполнена?
dmesg -T | grep -iE 'conntrack|table full' | tail
cat /proc/sys/net/netfilter/nf_conntrack_count /proc/sys/net/netfilter/nf_conntrack_max

# 3. Ресурсы: CPU, память, своп.
uptime; free -m; vmstat 1 5; docker stats --no-stream

# 4. Посторонний трафик: боты сканируют порты?
...
- всё чисто — писать хостеру: потери на их стороне видно только им.
